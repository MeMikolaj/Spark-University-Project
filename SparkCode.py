...
# If it's after 18:30, add 1 day to Date (after we added 5.5 hours it would be next day)
df1 = df.withColumn("Date", when(df.Time >= "18:30:00", date_add(df.Date, 1)).otherwise(df.Date))
# Add 5.5 hours to Time
df1 = df1.withColumn("Time", date_format(df.Time + expr('INTERVAL 5 HOURS 30 MINUTES'), 'HH:mm:ss'))
...
df4 = df1.orderBy(desc("Date")).groupBy("UserID").agg(max("Latitude").alias("North_Latitude"))
...
# Join 2 data frames above to find out a Day on which the North_Latitude has been achieved 
...
# Drop all the values that are less or equal to 0. We don't count going down and 0s are unneccesary.
df6 = df6.filter(df6.High_difference>0)
# Group the data by UserID and Data, sum up the distance climbed on each day for a User. 
df6 = df6.groupBy("UserID", "Date").agg(sum("High_difference").alias("Total_height"))
